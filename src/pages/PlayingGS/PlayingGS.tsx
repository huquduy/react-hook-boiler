...
const PlayingGS: React.FC = () => {
  ...
  const proxies = {
    ...
    toto4d: async () => {
      return `http://toto4dbet.com?token=${auth.token}`
    }
  }
  ...
  return (
    <div className='gs-page'>
      {isLoading && !errorLogin.length &&
        <Typography color="primary" variant="overline" display="block" gutterBottom={true}>
          Your game is loading ...
        </Typography>}
      {Boolean(errorLogin.length) &&
        <Typography color="secondary" variant="overline" display="block" gutterBottom={true}>
          {`${group}: ${errorLogin}`}
        </Typography>}
      <Loading />
    </div>
